...
export async function analyzeSEOHealth(url: string, userId?: string) {
    try {
        ...
        const brandInfo = await aiVisibilityService.extractBrandInfo(url);
        const strategicRecommendations = await aiVisibilityService.generateStrategicRecommendations(
            {
                visibilityScore: evaluation.healthScore,
                citationAuthority: Math.round(evaluation.healthScore * 0.8), // Correlation
                sentimentScore: 75 // Default
            },
            [], // Platform data not available in this flow yet
            brandInfo.brandName
        );
        ...
        const result = {
            ...
            strategicRecommendations, // Add AI Recommendations
            ...
            technicalAnalysis: {
                ...
                mobileOptimization: {
                    score: seoData.mobile.viewportExists ? 100 : 0,
                    status: seoData.mobile.viewportExists ? 'good' : 'poor'
                },
                securityHTTPS: {
