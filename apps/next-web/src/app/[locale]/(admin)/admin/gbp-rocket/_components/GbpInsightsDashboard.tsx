...
export default function GbpInsightsDashboard({ locationId }: { locationId: string | undefined }) {
    ...
    const [competitorForm, setCompetitorForm] = useState({ competitorName: '', rating: '', reviewCount: '', photoCount: '', estimatedVisibility: '' });
    ...
    const openAddDialog = () => {
        ...
        setCompetitorForm({ competitorName: '', rating: '', reviewCount: '', photoCount: '', estimatedVisibility: '' });
        setDialogOpen(true);
    };
    ...
    const openEditDialog = (comp: any) => {
        ...
        setCompetitorForm({
            ...
            rating: comp.rating?.toString() || '',
            reviewCount: comp.reviewCount?.toString() || '',
            photoCount: comp.photoCount?.toString() || '',
            estimatedVisibility: comp.estimatedVisibility?.toString() || ''
        });
        setDialogOpen(true);
    };

    const handleDeleteCompetitor = async (comp: any) => {
        if (!window.confirm(`Delete "${comp.competitorName}"?`)) return;

        try {
            await apiClient.delete(`${GBP_API_URL}/locations/${locationId}/competitors/${comp.id}`);
            setCompetitors(prev => prev.filter((c: any) => c.id !== comp.id));
        } catch (e) {
    ...
    const handleSubmitCompetitor = async () => {
        ...
        try {
            const payload = {
                ...
                rating: competitorForm.rating ? Number(competitorForm.rating) : undefined,
                reviewCount: competitorForm.reviewCount ? Number(competitorForm.reviewCount) : undefined,
                photoCount: competitorForm.photoCount ? Number(competitorForm.photoCount) : undefined,
                estimatedVisibility: competitorForm.estimatedVisibility ? Number(competitorForm.estimatedVisibility) : undefined,
            };

            if (editingCompetitor) {
                const res = await apiClient.put(`${GBP_API_URL}/locations/${locationId}/competitors/${editingCompetitor.id}`, payload);

                setCompetitors(prev => prev.map((c: any) => c.id === editingCompetitor.id ? res.data : c));
            } else {
                const res = await apiClient.post(`${GBP_API_URL}/locations/${locationId}/competitors`, payload);
    ...
    return (
        <>
                    ...
                    <Box sx={{ minWidth: 0 }}>
                        <Stack spacing={3}>
                            ...
                            <Card elevation={0} sx={{ border: '1px solid #E5E7EB' }}>
                                <CardContent sx={{ p: 4 }}>
                                    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                                        <Typography variant="h6" fontWeight="bold">{t('competitorBaseline')}</Typography>
                                        <Button size="small" variant="outlined" onClick={openAddDialog} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 600 }}>{t('addCompetitor')}</Button>
                                    </Stack>
                                                    ...
                                                    <TableRow key={comp.id} sx={{ '& td': { borderBottom: index === competitors.length - 1 ? 'none' : '1px solid #F3F4F6', py: 2.5 } }}>
                                                        ...
                                                        <TableCell align="right">
                                                            <Stack direction="row" spacing={1} justifyContent="flex-end">
                                                                <IconButton size="small" onClick={() => openEditDialog(comp)}><Edit sx={{ fontSize: 16, color: '#9CA3AF' }} /></IconButton>
                                                                <IconButton size="small" onClick={() => handleDeleteCompetitor(comp)}><Delete sx={{ fontSize: 16, color: '#EF4444' }} /></IconButton>
                                                            </Stack>
                                                        </TableCell>
                                                    </TableRow>
                                                ))}
                        ...
                        <Stack spacing={3} height="100%">
                            ...
                            <Card elevation={0} sx={{ border: '1px solid #E5E7EB' }}>
                                <CardContent sx={{ p: 4 }}>
                                    ...
                                    <Stack spacing={3}>
                                        {(() => {
                                            const vs = totals.visibilityScore || 0;
                                            const pvChange = changes.photoViews || 0;
                                            const strong = { rating: t('strong'), color: 'success', chipBg: '#D1FAE5', chipColor: '#065F46' };
                                            const moderate = { rating: t('moderate'), color: 'warning', chipBg: '#FEF3C7', chipColor: '#92400E' };
                                            const weak = { rating: t('weak'), color: 'error', chipBg: '#FEE2E2', chipColor: '#991B1B' };
                                            const neutral = { rating: t('na'), color: 'default', chipBg: '#F3F4F6', chipColor: '#6B7280' };
                                            const profileLevel = vs >= 75 ? strong : vs >= 50 ? moderate : vs > 0 ? weak : neutral;
                                            const photoLevel = pvChange > 5 ? strong : pvChange >= 0 ? moderate : weak;


                                            return [
                                                { label: t('postingFrequency'), ...neutral, tooltip: 'Post tracking not yet available' },
                                                { label: t('photoFreshness'), ...photoLevel, tooltip: `Photo views ${pvChange >= 0 ? '+' : ''}${pvChange}% vs previous period` },
                                                { label: t('reviewActivity'), ...neutral, tooltip: 'Review activity tracking not yet available' },
                                                { label: t('categoryCompleteness'), ...(competitors.length > 0 ? strong : moderate), tooltip: competitors.length > 0 ? 'Competitors tracked' : 'Add competitors to enhance this signal' },
                                                { label: t('profileCompleteness'), ...profileLevel, tooltip: `Visibility score: ${vs}` },
                                            ];
                                        })().map((signal, i) => {
                                            const { chipBg, chipColor } = signal as any;
                                            ...
                                            return (
                                                <Box key={i} display="flex" justifyContent="space-between" alignItems="center" p={1.5} bgcolor="#F9FAFB" borderRadius={2}>
                                                    <Stack direction="row" alignItems="center" spacing={1}>
                                                        ...
                                                        <MuiTooltip title={(signal as any).tooltip || ''}><InfoOutlined sx={{ fontSize: 14, color: '#9CA3AF', cursor: 'help' }} /></MuiTooltip>
                                                    </Stack>
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="xs" PaperProps={{ sx: { borderRadius: 3 } }}>
                <DialogTitle sx={{ fontWeight: 700 }}>{editingCompetitor ? 'Edit Competitor' : 'Add Competitor'}</DialogTitle>
                <DialogContent>
                    <Stack spacing={2} mt={1}>
                        <TextField
                            ...
                            onChange={e => setCompetitorForm(f => ({ ...f, competitorName: e.target.value }))}
                        />
                        <Stack direction="row" spacing={2}>
                            <TextField label="Rating" type="number" fullWidth size="small" inputProps={{ min: 0, max: 5, step: 0.1 }} value={competitorForm.rating} onChange={e => setCompetitorForm(f => ({ ...f, rating: e.target.value }))} />
                            <TextField label="Review Count" type="number" fullWidth size="small" inputProps={{ min: 0 }} value={competitorForm.reviewCount} onChange={e => setCompetitorForm(f => ({ ...f, reviewCount: e.target.value }))} />
                        </Stack>
                        <Stack direction="row" spacing={2}>
                            <TextField label="Photo Count" type="number" fullWidth size="small" inputProps={{ min: 0 }} value={competitorForm.photoCount} onChange={e => setCompetitorForm(f => ({ ...f, photoCount: e.target.value }))} />
                            <TextField label="Est. Visibility (0–100)" type="number" fullWidth size="small" inputProps={{ min: 0, max: 100 }} value={competitorForm.estimatedVisibility} onChange={e => setCompetitorForm(f => ({ ...f, estimatedVisibility: e.target.value }))} />
                        </Stack>
                    </Stack>
                </DialogContent>
                <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
                    <Button onClick={() => setDialogOpen(false)} sx={{ textTransform: 'none' }}>{t('cancel')}</Button>
                    <Button
                        variant="contained"
                        onClick={handleSubmitCompetitor}
                        disabled={submitting || !competitorForm.competitorName.trim()}
                        sx={{ textTransform: 'none', borderRadius: 2, fontWeight: 600, minWidth: 100 }}
                    >
                        {submitting ? <CircularProgress size={18} color="inherit" /> : editingCompetitor ? 'Save Changes' : 'Add'}
                    </Button>
                </DialogActions>
            </Dialog>
        </>
